...
def get_similarity_network(network_type, adata, threshold, k,
                           data_path, data_name, seed, train_gene_set_size,
...
def get_coexpression_network_from_train(adata, threshold, k, data_path,
                                        data_name, seed, train_gene_set_size,
                                        set2conditions):
    """
    ...
    fname = data_path + '/split_co_expression_network.csv'
    ...
    if os.path.exists(fname):
        ...
        X = adata.X
        train_tr = adata[adata.obs['split'] == 'train']
        train_tr = train_tr[train_tr.obs['control'] != 1]
        train_index = adata.obs_names.get_indexer(train_tr.obs['paired_control_index'].values)
        X_tr = X[train_index]
...
def filter_pert_in_go(condition, pert_names):
    ...
    Args:
        condition (str): whether condition is 'control' or not
    ...
    if condition == 'DMSO':
        ...
        num_ctrl = (cond1 == 'DMSO') + (cond2 == 'DMSO')
...
def loss_fct(pred, y, perts):
    ...
    for p in set(perts):
        ...
        losses = losses + mse_p(pred_p, y_p)
...
def get_mean_control(adata):
    ...
    """
    mean_ctrl_exp = adata[adata.obs['condition'] == 'DMSO'].to_df().mean()
...
def get_genes_from_perts(perts):
    ...
    gene_list = [g for g in gene_list if g != 'DMSO']
